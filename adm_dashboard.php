...
try {
    ...
    // ==========================================
    // ĐÃ FIX: Lấy danh sách bệnh nhân chưa thanh toán ở TẤT CẢ trạng thái khám (ngoại trừ Cancelled)
    // ==========================================
    $stmtFee = $pdo->query("
        SELECT u_p.full_name as patient_name, u_p.user_id, COUNT(a.appointment_id) as unpaid_bills
        FROM Appointments a
        JOIN Users u_p ON a.patient_id = u_p.user_id
        WHERE a.status != 'Cancelled' AND a.fee_status = 'Unpaid'
                                        ...
                                        <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td class="py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div class="w-8 h-8 rounded-full <?php echo getAvatarColor($appt['patient_name']); ?> text-white flex items-center justify-center font-bold text-xs shadow-sm">
                                                        <?php echo getInitials($appt['patient_name']); ?>
                                                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="flex justify-between items-center p-4 border border-gray-100 bg-gray-50/30 hover:bg-red-50/30 hover:border-red-100 rounded-xl transition-all group">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="w-10 h-10 rounded-full <?php echo getAvatarColor($fee['patient_name']); ?> text-white flex items-center justify-center font-bold text-sm shadow-sm">
                                                <?php echo getInitials($fee['patient_name']); ?>
                                            </div>
