...
function autoLink($text) {
    $pattern = '/(https?:\/\/[^\s]+)/';
    $replacement = '<a href="$1" target="_blank" class="underline font-bold text-yellow-300 hover:text-white transition-colors">$1</a>';
                ...
                <div class="w-[340px] bg-gray-50/50 border-r border-gray-100 flex flex-col flex-shrink-0">
                    ...
                    <div class="flex-1 overflow-y-auto p-3 space-y-1">
                        ...
                        <?php else: ?>
                            ...
                            class="chat-item flex items-center gap-3 p-3 rounded-xl transition-all border <?php echo $receiverId == $chat['user_id'] ? 'bg-blue-50/60 border-blue-200 shadow-sm' : 'border-transparent hover:bg-white hover:border-gray-100 hover:shadow-sm'; ?>"
                            data-name="<?php echo strtolower(htmlspecialchars($chat['full_name'])); ?>">
                    ...
                    <?php if ($receiverId && $receiverInfo): ?>
                        <div class="h-[76px] bg-white border-b border-gray-100 px-8 flex items-center justify-between flex-shrink-0 shadow-[0_4px_20px_-15px_rgba(0,0,0,0.1)] z-10">
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <div class="w-10 h-10 rounded-full <?php echo getAvatarColor($receiverName); ?> text-white flex items-center justify-center text-sm font-bold shadow-sm">
                                        <?php echo getInitials($receiverName); ?>
                                    </div>
                                </div>
                                <div>
                            ...
                            <div class="flex gap-3">
                                <button class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors"></button>
                                <button class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors"></button>
                            </div>
                        </div>
...
<script>
    ...
    function searchChat() {
        ...
        items.forEach(item => {
            // Lấy tên người chat từ data-name đã lưu ở thẻ <a>
            let name = item.getAttribute('data-name');
            // Nếu tên có chứa chữ vừa gõ -> Hiện, không thì -> Ẩn
