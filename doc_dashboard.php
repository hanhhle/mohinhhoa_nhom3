...
// --- CHỖ NÀY ĐÃ ĐƯỢC CỐ ĐỊNH ẢNH CỦA BẠN ---
$doctorAvatar = 'img/hoang-anh.jpg'; 
            ...
            <header class="h-[72px] bg-white border border-gray-100 rounded-2xl shadow-sm flex items-center justify-between px-6">
                ...
                <div class="flex items-center gap-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow-sm border border-white flex-shrink-0">
        ...
        <div class="flex-1 px-10 pb-10 overflow-y-auto">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 mb-8 overflow-hidden">
                <div class="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
                    ...
                    <a href="doc_appointments.php" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-blue-50 text-blue-500 transition-colors">
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-blue-50/20 transition-colors border-b border-gray-50 last:border-0">
                                ...
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white bg-gradient-to-br from-emerald-500 to-teal-600 shadow-sm overflow-hidden">
                                            <?php 
                                            $pNameParts = explode(' ', trim($appt['patient_name']));
                                            $pInitials = mb_strtoupper(mb_substr($pNameParts[0], 0, 1, 'UTF-8'), 'UTF-8');
                                            if (count($pNameParts) > 1) { $pInitials .= mb_strtoupper(mb_substr(end($pNameParts), 0, 1, 'UTF-8'), 'UTF-8'); }

                                            if (!empty($appt['p_avatar']) && file_exists($appt['p_avatar'])): 
                                            ?>
                                                <img src="<?php echo $appt['p_avatar']; ?>" class="w-full h-full object-cover" onerror="this.style.display='none';">
                                            <?php else: ?>
                                                <span><?php echo $pInitials; ?></span>
                                            <?php endif; ?>
                                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
                    ...
                    <a href="doc_messages.php" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-blue-50 text-blue-500 transition-colors">
                        <i class="fa-solid fa-arrow-up-right-from-square"></i>
                    </a>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 w-full">
                            ...
                            <a href="doc_messages.php?receiver_id=<?php echo $msg['sender_id']; ?>" class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-sm hover:bg-blue-50/10 transition-all group">
                                <div class="relative flex-shrink-0">
                                    <div class="w-12 h-12 rounded-full flex items-center justify-center text-base font-bold text-white bg-gradient-to-br from-indigo-500 to-purple-600 overflow-hidden">
                                        <?php 
                                        $sNameParts = explode(' ', trim($msg['sender_name']));
                                        $sInitials = mb_strtoupper(mb_substr($sNameParts[0], 0, 1, 'UTF-8'), 'UTF-8');
                                        if (count($sNameParts) > 1) { $sInitials .= mb_strtoupper(mb_substr(end($sNameParts), 0, 1, 'UTF-8'), 'UTF-8'); }

                                        if (!empty($msg['sender_avatar']) && file_exists($msg['sender_avatar'])): 
                                        ?>
                                            <img src="<?php echo $msg['sender_avatar']; ?>" class="w-full h-full object-cover" onerror="this.style.display='none';">
                                        <?php else: ?>
                                            <span><?php echo $sInitials; ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if(!$msg['is_read']): ?>
