...
// CHỨC NĂNG: Bệnh nhân xem Bệnh án (Read-only) và Cập nhật Tiền sử bệnh
// ==========================================
session_start();
require 'db.php';
...
// 1. XỬ LÝ THÊM MỚI MEDICAL HISTORY VÀO BẢNG CHUẨN
...
// 2. XỬ LÝ XÓA MEDICAL HISTORY (NẾU BỆNH NHÂN NHẬP SAI)
...
$patientInfo = null;
try {
    // Thông tin cá nhân (Bây giờ chỉ để View, không update)
    ...
    // Danh sách Tiền sử bệnh từ bảng Medical_History
    ...
    // Lịch sử Khám (Chỉ lấy Completed để hiển thị chẩn đoán)
    ...
    // Nếu có lịch sử khám, lấy cuộc hẹn mới nhất để hiển thị chi tiết EMR bên cột phải
